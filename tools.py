...
def web_search(query: str):
    ...
    search_attempts = [
        ...
        (
            {"query": cleaned_query.split()[-3:], "max_results": 3}
    ...
    for attempt_num, attempt in enumerate(search_attempts, 1):
        try:
            query_to_use = (
                " ".join(attempt["query"])
                if isinstance(attempt["query"], list)
                else attempt["query"]
            )
            max_results = attempt["max_results"]

            print(
                f"嘗試 {attempt_num}: 搜索 '{query_to_use}' (最大結果: {max_results})"
            )
...
"""
                    ...
                    print(
                        f"=== 搜索成功，返回 {len(formatted_results)} 個格式化結果 ==="
                    )
                    return final_result
