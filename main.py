...
from rag_setup import rag_manager, setup_rag
...
# 內容清理函數
def sanitize_content_for_json(content: str) -> str:
    """
    清理內容以確保JSON序列化的安全性
    """
    if not content:
...
def safe_json_dumps(data: Dict[str, Any]) -> str:
    ...
    try:
        ...
        if "content" in data and isinstance(data["content"], str):
            data = data.copy()
            data["content"] = sanitize_content_for_json(data["content"])

        # 使用ensure_ascii=True確保所有非ASCII字符都被轉義
        return json.dumps(data, ensure_ascii=True, separators=(",", ":"))
    except Exception as e:
        # 如果序列化失敗，返回錯誤訊息
        print(f"JSON序列化失敗: {e}")
        return json.dumps(
            {"type": "error", "message": f"內容序列化失敗: {str(e)}"}, ensure_ascii=True
        )
...
記住：你是RAG系統，資訊的可靠性和來源透明度是你的核心價值。"""
    ...
    try:
        ...
        for chunk in first_response_stream:
            ...
            if hasattr(chunk, "text") and chunk.text:
                ...
                json_data = safe_json_dumps({"type": "content", "content": chunk.text})
        ...
        if not function_calls:
            if not has_content:
                json_data = safe_json_dumps(
                    {
                        "type": "content",
                        "content": "我理解了你的問題，但目前沒有需要額外資訊來回答。",
                    }
                )
                yield f"data: {json_data}\n\n"
            json_data = safe_json_dumps(
                {"type": "agent_finish", "final_answer": collected_content}
            )
            yield f"data: {json_data}\n\n"
            return
        ...
        json_data = safe_json_dumps(
            {"type": "error", "message": f"與 Gemini 互動時發生錯誤: {str(e)}"}
        )
        yield f"data: {json_data}\n\n"
        return
    ...
    valid_function_calls = [
        fc for fc in function_calls if fc is not None and hasattr(fc, "name")
    ]
    ...
    for function_call in valid_function_calls:
        ...
        try:
            ...
            json_data = safe_json_dumps(event)
            yield f"data: {json_data}\n\n"
            ...
            if tool_name in available_tools:
                ...
                json_data = safe_json_dumps({"type": "error", "message": error_msg})
                yield f"data: {json_data}\n\n"
                return
    ...
    if function_responses:
        gemini_messages.append({"role": "function", "parts": function_responses})
    ...
    try:
        ...
        error_msg = f"在整合工具結果時發生錯誤: {str(e)}"
        json_data = safe_json_dumps({"type": "error", "message": error_msg})
        yield f"data: {json_data}\n\n"
